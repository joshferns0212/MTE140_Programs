...
public:
	...
	//HELPERS
	//PURPOSE: Recursive function called by destructor
	void delete_BST(TaskItem* curr);

	//PURPOSE: Called by height() to find tree height
	unsigned int max_height(TaskItem* curr) const;

	//PURPOSE: Called by print() to print every node
	void traverse_BST(TaskItem* curr) const;

	//PURPOSE: Called by exists() to find if a node exists
	bool search_BST(BinarySearchTree::TaskItem val, BinarySearchTree::TaskItem* curr) const;
};
#endif
